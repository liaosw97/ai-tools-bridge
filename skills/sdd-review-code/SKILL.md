...
| 选项 | 行为 |
|------|------|
| 自动修复 | 按建议使用 Edit 工具修改文件。成功 → 标记已处理，继续下一个；失败 → 降级为手动修复 |
| 手动修复 | 输出修复指引（文件路径、修改位置、预期内容），等待用户确认。验证修改是否完成：完整 → 标记已处理，继续下一个；不完整 → 提示具体缺失项，询问是否继续（忽略/重新修改） |
| 跳过 | 记录已跳过，继续下一个 |
| 标记 | 记录已标记，继续下一个 |
