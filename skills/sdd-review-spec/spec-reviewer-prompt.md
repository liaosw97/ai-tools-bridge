...
> 注：跨模块一致性的判定依赖审查员对项目结构和 spec 间引用关系的分析，不做硬性断言。所有检查项的结论均为基于分析的判断，需人工确认。对于单模块项目（specs/ 下 0-1 个子目录），本维度自动通过——Approved 清单中该维度标记为 `[x]` 并附注"N/A"，且不因此维度产生任何 issue，报告中标注"单模块项目，跨模块一致性维度不适用"。
