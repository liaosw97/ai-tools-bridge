...
> 与代码审查阶段通过 git diff 检测不同，spec 审查阶段尚无代码变更，因此从提案文档推断工作类型。

从 proposal.md 或 brainstorm.md 判断当前变更是否涉及 skill 开发：
- 检查变更描述是否包含 "skill"、"SKILL.md"、"技能定义" 等关键词
- 检查影响分析中的文件路径是否包含 `skills/` 目录

#### 扫描调度

```
涉及 skill 开发 且 skill-craft-adapter 可用
  → 调用 skill-craft-adapter:skill-check 进行 spec 质量规范扫描
  → 结果写入 reviews/scan-r<N>.md
不涉及 或 skill-craft-adapter 不可用
  → SKIPPED，不生成扫描报告
```
...
规范扫描: SCANNED / SKIPPED
  SCANNED: 已调用 skill-craft-adapter:skill-check
